...
import config from "./cursor/config.js";
import { BridgeWsClient } from "./cursor/ws-client.js";
import { JobQueue } from "./shared/queue.js";

const log = createLogger("bridge-cursor");

async function main(): Promise<void> {
  log.info(`Bridge-cursor starting (MCP: ${config.automationMcpPath})`);
  ...
  if (!config.cursorApiKey) {
    log.info("No CURSOR_API_KEY in env — waiting for API key from web app credentials panel");
  }
