...
type SettingsTab = "connection" | "templates";
...
const NAV_ITEMS: { id: SettingsTab; label: string; icon: typeof Plug }[] = [
  { id: "connection", label: "Connection & Credential", icon: Plug },
  { id: "templates", label: "Template Prompt", icon: FileText },
];
...
export function SettingsModal({ open, onClose, connection, templates }: SettingsModalProps) {
      ...
      >
        ...
        <div className="flex shrink-0 gap-1 border-b border-white/8 p-2 md:hidden">
          {NAV_ITEMS.map(({ id, icon: Icon }) => (
            ...
            >
              ...
              <span className="truncate">{id === "connection" ? "Connection" : "Templates"}</span>
            </button>
          ))}
          <Button
            type="button"
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-2 sm:px-6 sm:py-4">
            {activeTab === "connection" ? connection : templates}
          </div>
        </div>
      </div>
    </div>,
